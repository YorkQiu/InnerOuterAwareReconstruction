...
import os
import yaml
# from inference_3CE_TUM_RGBD import TUMRGBDSet
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tumrgbd-dir", type=str)
    parser.add_argument("--mesh-dir", type=str)
    args = parser.parse_args()

    TUM_test_set = {d['name']: d for d in TUMRGBDSet(args.tumrgbd_dir, args.mesh_dir)}
    ...
    for pred_plyfile in tqdm.tqdm(pred_plyfiles, desc='Pred Files'):
        ...
        tum_data = TUM_test_set[scene_name]

        # load pred mesh and gt mesh
        pred_mesh_o3d = o3d.io.read_triangle_mesh(pred_plyfile)
        gt_mesh_o3d = o3d.io.read_triangle_mesh(tum_data['gt_mesh'])
        gt_mesh_o3d.compute_vertex_normals()

        # load tsdf data
        # gt_tsdf = tum_data['gt_tsdf']
        # gt_tsdf_vol = gt_tsdf['tsdf']
        # origin = gt_tsdf['vol_origin']
        # voxel_size = gt_tsdf['voxel_size']

        tsdf_info = tum_data['tsdf_info']
        ...
        rgb_imgfiles = tum_data['rgb_files']
        depth_imgfiles = tum_data['depth_files']
        pose = tum_data['extrinsic']
        intr = tum_data['intrinsic']
