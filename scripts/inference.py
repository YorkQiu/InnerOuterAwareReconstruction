...
def load_model(ckpt_file, use_proj_occ, config):
    model = lightningmodel.LightningModel.load_from_checkpoint(ckpt_file, config=config)
...
def get_img_feats(IOAR, imheight, imwidth, proj_mats, rgb_imgfiles, cam_positions):
...
def inference(model, info_file, outfile, n_imgs, cropsize, config):
    ...
    scene_minbound, scene_maxbound = get_scene_bounds(pose, intr, imheight, imwidth, frustum_depth=4)
    ...
    for resname, res in model.vortx.resolutions.items():
        ...
        for tile in tiles:
            ...
            tile["voxel_features_occ"] = torch.cat(
                (tile["voxel_features_occ"], bp_feats.cpu(), tile["voxel_logits_occ"]),
                dim=-1,
            )
        ...
        voxel_features_occ = torchsparse.SparseTensor(
            torch.cat([tile["voxel_features_occ"] for tile in tiles], dim=0).cuda(),
            torch.cat([voxel_inds, voxel_batch_inds], dim=-1).cuda(),
        )

        voxel_features = model.vortx.cnns3d[resname](voxel_features)
        voxel_features_occ = model.vortx.cnns3d_occ[resname](voxel_features_occ)
        voxel_logits = model.vortx.output_layers[resname](voxel_features)
        voxel_logits_occ = model.vortx.output_layers_occ[resname](voxel_features_occ)
        ...
        if resname in ["coarse", "medium"]:
            ...
            voxel_inds = model.vortx.upsampler.upsample_inds(voxel_logits_occ.C[occupancy])
...
if __name__ == "__main__":
    ...
    with torch.cuda.amp.autocast():
        ...
        for info_file in tqdm.tqdm(info_files):
            ...
            try:
                mesh = inference(model, info_file, outfile, args.n_imgs, cropsize, config)
