...
def load_info_files(scannet_dir, split):
    ...
    info_files = [f for f in info_files if os.path.basename(os.path.dirname(f)) in scene_names]
    return info_files


def load_pseudo_info_files(scannet_dir, split, info_name):
    with open(os.path.join(scannet_dir, f"scannetv2_{split}.txt"), "r") as f:
        scene_names = f.read().split()
    scan_dir = "scans" if split in ["train", "val"] else "scans_test"
    info_files = sorted(glob.glob(os.path.join(scannet_dir, scan_dir, "*", info_name)))
    # only load info files
    info_files = [f for f in info_files if os.path.basename(os.path.dirname(f)) in scene_names]
    return info_files
