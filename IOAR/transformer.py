...
class EncoderBlock(torch.nn.Module):
    def __init__(self, in_dim, num_heads, mlp_dim, dropout_rate=0.0, attn_dropout_rate=0.0):
        super().__init__()
    ...
    """
    x: (n_imgs, n_voxels, C), masks: (num_heads * n_voxels, n_imgs, n_imgs)
    """
...
class Transformer(torch.nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            layer = EncoderBlock(in_dim, num_heads, mlp_dim, dropout_rate, attn_dropout_rate)
            self.encoder_layers.append(layer)

        self.num_heads = num_heads

    """
    x: (n_imgs, n_voxels, C), mask: (n_voxels, n_imgs, n_imgs)
    """
    def forward(self, x, mask=None):
        if self.num_heads > 1:
            b, s, t = mask.shape
